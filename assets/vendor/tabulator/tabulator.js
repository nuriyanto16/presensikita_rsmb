...
;(function (global, factory) {
	...
	Page.prototype._setPageButtons = function () {
        ...
        self.pagesElement.appendChild(self._generatePageButton(1)); //first

		// check elipsis
		min = min + 1; max = max - 1;
        if ((min - 1) > 1) {
            self.pagesElement.appendChild(self._generatePageElipsisButton());
            min = min + 1;
        }
        let isCreateElipsis = false;
        if ((max + 1) < this.max) {
            isCreateElipsis = true;
        	max = max - 1;
        }
		...
		if (isCreateElipsis) self.pagesElement.appendChild(self._generatePageElipsisButton());
        if (self.max > 1) self.pagesElement.appendChild(self._generatePageButton(self.max)); //last
    ...
    Page.prototype._generatePageElipsisButton = function () {
        let button = document.createElement("button");
        button.classList.add("tabulator-page", 'disabled');
        button.setAttribute("type", "button");
        button.setAttribute("role", "button");
        button.textContent = "...";
