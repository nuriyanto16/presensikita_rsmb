...
$(document).ready(function () {
    ...
    let elJenisReimburseId = $("#jenis_reimburse_id");
    ...
    elJenisReimburseId.select2({
        allowClear: false,
        placeholder: "- Pilih -"
    });
    elJenisReimburseId.val(elJenisReimburseId.attr('value')).trigger('change');

    function build_employee(p_COMPID) {
        if (p_COMPID == null) p_COMPID = elCompanyId.val();
        elEmployeeId.html('');

        $.post(base_url + "presensi/gantibiaya/get_node_employee", { COMPID: p_COMPID  }, function (data) {
