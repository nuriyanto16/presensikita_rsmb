...
class LaporanInput extends REST_Controller
{
	...
	public function index_post()
	{
		...
		if ($nik !== null || $comp_code !== null || $id_abs_type !== null || $remark !== null || 
        	...
        	while ( $x < $cnt_file) {
        		...
        		$imageGenerate = $str[1];
        		if($stat_generate == 1){
        			$new_name = $this->generateImage($imageGenerate,'png','LH',$nik,$comp_code);
        		}else{
        	...
        	//UPLOAD FILE
        	$periode = date("Y");
        	$execute = $this->mod->InsUpdLaporan($pengajuan_id, $nik, $comp_code, $id_abs_type, $remark, $periode, 
	...
	public function edit_get()
	{
		...
		if ($pengajuan_id !== null) {
        	...
        	$status = 1;
			$row = $this->mod->getLaporanEdit($pengajuan_id);
		...
		if ($row) {
    		...
    		$build_array = array();
    		$linkpath = base_url().'uploads/laporanharian/'.$comp_code.'/';
    ...
    public function type_get()
	{
		...
		if ($nik !== null || $comp_code !== null){
			$res = $this->mod->getLaporanType($nik,$comp_code);
		}
    ...
    {
        $folderPath =  upload_path.'/laporanharian/'.$comp_code.'/';
