...
class IzinInput extends REST_Controller
{
	...
	public function index_post()
	{
		...
		if ($nik !== null || $comp_code !== null || $id_abs_type !== null || $remark !== null || 
        	...
        	while ( $x < $cnt_file) {
        		...
        		$imageGenerate = $str[1];
        		if($stat_generate == 1){
        			$new_name = $this->generateImage($imageGenerate,'png','IZIN',$nik,$comp_code);
        		}else{
        	...
        	//UPLOAD FILE
        	$periode = date("Y");
        	$execute = $this->mod->InsUpdizin($pengajuan_id, $nik, $comp_code, $id_abs_type, $remark, $periode, 
	...
	public function edit_get()
	{
		...
		if ($pengajuan_id !== null) {
        	...
        	$status = 1;
			$row = $this->mod->getIzinEdit($pengajuan_id);
		...
		if ($row) {
    		...
    		$build_array = array();
    		$linkpath = base_url().'uploads/izin/'.$comp_code.'/';
	        foreach ($DataAttach as $row_attach) {
	            array_push($build_array,
	                array(
	                	...
	                	"link" => $linkpath.$row_attach->URL_ATC_IZIN,
	                	"base64" => $row_attach->URL_ATC_IZIN,
	                	"is_new" => 0
	                )
	            );
	        }
			...
			$this->response([
            	...
            	"remark" => $row->ALASAN_IZIN,
            	"periode" => $periode,
            	"start_date" => $row->TGL_AWAL_IZIN,
            	"end_date" => $row->TGL_AKHIR_IZIN,
                "jml" => $row->JML,
    ...
    public function type_get()
	{
		...
		if ($nik !== null || $comp_code !== null){
			$res = $this->mod->getIzinType($nik,$comp_code);
		}
    ...
    {
        $folderPath =  upload_path.'/izin/'.$comp_code.'/';
