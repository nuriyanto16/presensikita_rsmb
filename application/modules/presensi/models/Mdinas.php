 ...
 * Class Mdinas
 */
class Mdinas extends Mst_model
{
    ...
    function get($id = null, $offset = null, $limit = null, $order = null, $filters = null)
    {
        $this->db->select("a.id_aju, date_format(a.tgl_aju, '%d-%m-%Y') AS tgl_aju, 
                            ...
                            d.tujuan, d.keperluan, d.nm_pejabat, d.jabatan,
                            d.all_bdgjkt, d.all_lr_kota, d.all_lr_negeri, 
                            d.tr_k_pribadi, d.tr_k_dinas, d.tr_ka, d.tr_pesawat, d.tr_travel,
                            d.tr_bus, d.ak_hotel, d.ak_hotel_nom, d.ak_hotel_ket, d.ak_tr_loc, 
                            d.ak_tr_loc_nom, d.ak_tr_loc_ket, d.ak_susp, d.ak_susp_nom, d.ak_susp_ket,
                            date_format(d.tgl_brkt, '%d-%m-%Y') AS tgl_brkt, 
                            date_format(d.tgl_plng, '%d-%m-%Y') AS tgl_plng,
                            DATEDIFF(d.tgl_plng, d.tgl_brkt) + 1 AS jml,
                            a.app_nik, a.app_ket");
        $this->db->from("{$this->table} a");
        $this->db->join('z_jns_aju b','a.jns_aju=b.jns_aju');
        $this->db->join('z_r_jalandinas d','a.id_aju=d.id_aju');
    ...
    function getExport($start_date = null , $end_date = null)
    {
        ...
        if ($start_date && $end_date) {
            $this->db->group_start()
                ->group_start()
                    ->where("d.tgl_brkt >= ", $start_date)
                    ->where("d.tgl_brkt <= ", $end_date)
                    ->where("a.active", 1)
                ->group_end()
                ->or_group_start()
                    ->where("d.tgl_plng >= ", $start_date)
                    ->where("d.tgl_plng <= ", $end_date)
                    ->where("a.active", 1)
                ->group_end()
            ->group_end();
        }
