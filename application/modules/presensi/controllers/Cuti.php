...
class Cuti extends Mst_controller
{
    ...
    function __construct()
    {
        ...
        $this->MOD_ALIAS = "MOD_PRESENSI_CUTI";
    ...
    {
        $this->data['titlehead'] = "Cuti";

        $loadfoot['javascript'] = array(
            HTTP_MOD_JS . 'modules/presensi/cuti.js'
        );
        ...
        $this->_render_page('vcuti_list', $this->data, false, 'tmpl/vwbacktmpl');
    }
...
//                anchor("reference/position/activate/{$id}    ", "<i class='fa fa-times text-red'>&nbsp;</i>", array("onclick" => "return confirm('Aktifkan data ?');"));
    ...
    public function edit_form()
    {
        ...
        if ($this->data['id'] != "") {
            ...
            $this->data['titlehead'] = "Input Cuti";
        }
        ...
        $stdClass->alasan_cuti = null;
        $stdClass->periode = date("Y");
        $stdClass->tgl_awal_cuti = null;
        $stdClass->tgl_akhir_cuti = null;        
        $stdClass->jml = 0;
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $stdClass->alasan_cuti = $this->input->post('remark');
            $stdClass->periode = date("Y");
            $stdClass->tgl_awal_cuti = $this->input->post('start_date');
            $stdClass->tgl_akhir_cuti = $this->input->post('end_date');
            ...
            if ($this->form_validation->run($this) === TRUE) {
                ...
                $remark = $stdClass->alasan_cuti;
                $periode = $stdClass->periode;
                $start_date = $stdClass->tgl_awal_cuti;
                $end_date = $stdClass->tgl_akhir_cuti;
                $jml = $stdClass->jml;
                $cnt_file = 0;
                $cnt_file_old = 0;
                $cnt_exist = 0;
                ...
                if (in_array($action, array('save'))) {
                    ...
                    if(!empty($fileNames)){ 
                        ...
                        foreach($_FILES['images']['name'] as $key=>$val){ 
                            ...
                            $targetDir =  upload_path.'cuti/'.$comp_code.'/';
               ...
               //check to see if we are updating data
                ...
                if ($this->input->post('id')) { // update
                    $isUpdate = true;
                    if ($action === 'save') {
                        $isError = ! $this->mcti->InsUpdCuti($pengajuan_id, $nik, $comp_code, $id_abs_type, $remark, $periode, 
                        $date, $this->_fyyyymmdd($start_date), $this->_fyyyymmdd($end_date), 
                        $jml, $cnt_file, $params_image_input);

                        if (!$isError) {
                            $this->mcommon->setLog($this->get_userid(), $this->MOD_ALIAS, $id, "Izin Diperbaharui");
                        }
                    ...
                    $execute = $this->mcti->InsUpdCuti($pengajuan_id, $nik, $comp_code, $id_abs_type, $remark, $periode, 
                        $date, $this->_fyyyymmdd($start_date), $this->_fyyyymmdd($end_date), 
                        $jml, $cnt_file, $params_image_input);
                ...
                if ($isError) {
                    ...
                    redirect("presensi/cuti", 'refresh');
                }
            } else {
        ...
        $loadfoot['javascript'] = array(
            HTTP_MOD_JS . 'modules/presensi/cuti_form.js',
        ...
        $jenisizin = $this->mod->getCutiType(null, null);
        $list_jeniscuti[null] = "Pilih Tipe Cuti";
        foreach ($jenisizin as $rowjns) {
            $list_jeniscuti[$rowjns->CUTI_ID] = $rowjns->CUTI_DESC;
        }

        $this->data['id_abs_type'] = array(
            'name' => 'id_abs_type',
            'id' => 'id_abs_type',
            'options' => $list_jeniscuti,
        ...
        $this->data['start_date'] = array(
            ...
            'type' => 'text',
            'class' => 'form-control',
            'value' => $this->form_validation->set_value('start_date', $stdClass->tgl_awal_cuti)
        );
        ...
        $this->data['end_date'] = array(
            ...
            'type' => 'text',
            'class' => 'form-control',
            'value' => $this->form_validation->set_value('end_date', $stdClass->tgl_akhir_cuti)
        );
        ...
        $this->data['remark'] = array(
            ...
            'maxLength' => '500',
            'rows' => '2',
            'value' => $this->form_validation->set_value('remark', $stdClass->alasan_cuti)
        );
        ...
        if($id!="0"){
            $linkpath = base_url().'uploads/cuti/'.$stdClass->comp_code.'/';
            foreach ($DataAttach as $row_attach) {
                array_push($build_array,
                    array(
                        ...
                        "src" => $linkpath.$row_attach->URL_ATC_CUTI,
                        "is_new" => 0
                    )
                );
            }
        ...
        return $this->_render_page('presensi/vcuti_form', $this->data, false, 'tmpl/vwbacktmpl');
    }
    ...
    public function activate($id)
    {
        ...
        $data['active'] = 1;
        $activation = $this->mcti->update($id, $data);
        if ($activation) {
    ...
    public function deactivate($id = NULL)
    {
        ...
        $data['active'] = 0;
        $deactivate = $this->mcti->update($id, $data);
        if ($deactivate) {
    ...
    public function getExportExcel($fromDate=null, $toDate=null) {
        ...
        // DETAIL
        $results = $this->mcti->getExport($start_date, $end_date);
        ...
        $html = $this->load->view("vcuti_export", $this->data, true);
