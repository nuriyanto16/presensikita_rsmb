 ...
 * @property Mpengobatan $mobat
 */

class Pengobatan extends Mst_controller
{
    ...
    function __construct()
    {
        ...
        $this->MOD_ALIAS = "MOD_PRESENSI_PENGOBATAN";
        ...
        $this->load->model("Mpengobatan", "mobat");
    ...
    {
        $this->data['titlehead'] = "Pengobatan";

        $loadfoot['javascript'] = array(
            HTTP_MOD_JS . 'modules/presensi/pengobatan.js'
        );
        ...
        $this->_render_page('vpengobatan_list', $this->data, false, 'tmpl/vwbacktmpl');
    }
    ...
    public function lists()
    {
        ...
        $results = $this->mobat->get(null, $start, $limit, $order, $filters);
        $totalfiltered = $this->mobat->get_cnt($filters);
        $totaldata = $this->mobat->get_cnt();
        ...
        foreach ($results as $row) {
            ...
            if ($this->_edit) {
                $atr_edit['title'] = 'Edit';
                $atr_edit['url'] = 'presensi/pengobatan/edit_form/';
                $atr_edit['class'] = '';
            }
            if ($this->_delete) {
                $atr_del['title'] = 'Hapus';
                $atr_del['url'] = 'presensi/pengobatan/delete/';
                $atr_del['class'] = '';
...
//                anchor("reference/position/activate/{$id}    ", "<i class='fa fa-times text-red'>&nbsp;</i>", array("onclick" => "return confirm('Aktifkan data ?');"));
            ...
            $obj['nama_kuitansi'] = $row->nama_kuitansi;
            $obj['diagnosa'] = $row->diagnosa;
            $obj['nom_kuitansi'] = $this->to_rupiah($row->nom_kuitansi);
            $obj['nilai_diganti'] = $this->to_rupiah($row->nilai_diganti);
    ...
    public function edit_form()
    {
        ...
        if ($this->data['id'] != "") {
            ...
            $this->data['titlehead'] = "Edit Pengobatan";
        } else {
            $this->data['titlehead'] = "Input Pengobatan";
        }
        ...
        $stdClass->jns_aju = null;
        $stdClass->tgl_aju = null;  
        $stdClass->nama_kuitansi = null;
        $stdClass->tgl_kuitansi = null;
        $stdClass->diagnosa = null;
        $stdClass->nom_kuitansi = 0;
        $stdClass->nilai_diganti = 0;
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $stdClass->pengobatan_id = $this->input->post('pengobatan_id');
            $stdClass->nama_kuitansi = $this->input->post('nama_kuitansi');
            $stdClass->diagnosa = $this->input->post('diagnosa');
            ...
            if ($this->form_validation->run($this) === TRUE) {
                ...
                $pengobatan_id = $stdClass->pengobatan_id;
                $diagnosa = $stdClass->diagnosa;
                $periode = $stdClass->periode;
                $tgl_kuitansi = $stdClass->tgl_kuitansi;
                $nama_kuitansi = $stdClass->nama_kuitansi;
                $nom_kuitansi = $stdClass->nom_kuitansi;
                ...
                if (in_array($action, array('save'))) {
                    ...
                    if(!empty($fileNames)){ 
                        ...
                        foreach($_FILES['images']['name'] as $key=>$val){ 
                            ...
                            $targetDir =  upload_path.'pengobatan/'.$comp_code.'/';
               ...
               //check to see if we are updating data
                ...
                if ($this->input->post('id')) { // update
                    $isUpdate = true;
                    if ($action === 'save') {
                        // $isError = ! $this->mobat->InsUpdCuti($pengajuan_id, $nik, $comp_code, $pengobatan_id, $diagnosa, $periode, 
                        // $date, $this->_fyyyymmdd($tgl_kuitansi), $this->_fyyyymmdd($tgl_aju), 
                        // $jml, $cnt_file, $params_image_input);

                        $isError = ! $this->mod->InsUpdPengobatan($pengajuan_id, $nik, $comp_code, $periode,
                                                      $this->_fyyyymmdd($tgl_kuitansi), 
                                                      $pengobatan_id, $nama_kuitansi, 
                                                      $diagnosa, $nom_kuitansi, $nilai_diganti, 
                                                      $cnt_file, $params_image_input);


                        if (!$isError) {
                            $this->mcommon->setLog($this->get_userid(), $this->MOD_ALIAS, $id, "Pengobatan Diperbaharui");
                        }
                    ...
                    // $execute = $this->mobat->InsUpdCuti($pengajuan_id, $nik, $comp_code, $pengobatan_id, $diagnosa, $periode, 
                    //     $date, $this->_fyyyymmdd($tgl_kuitansi), $this->_fyyyymmdd($tgl_aju), 
                    //     $jml, $cnt_file, $params_image_input);

                    $execute = $this->mod->InsUpdPengobatan($pengajuan_id, $nik, $comp_code, $periode,
                        $this->_fyyyymmdd($tgl_kuitansi), 
                        $pengobatan_id, $nama_kuitansi, 
                        $diagnosa, $nom_kuitansi, $nilai_diganti, 
                        $cnt_file, $params_image_input);
                    ...
                    if ($execute) {
                        ...
                        $this->mcommon->setLog($this->get_userid(), $this->MOD_ALIAS, $id, "Pengobatan Baru");
                    }
                ...
                if ($isError) {
                    ...
                    redirect("presensi/pengobatan", 'refresh');
                }
            } else {
        ...
        $loadfoot['javascript'] = array(
            ...
            HTTP_MOD_JS . 'modules/presensi/pengobatan_form.js',
        ...
        // select nama kuitansi
        $this->data['nama_kuitansi'] = array(
            'name' => 'nama_kuitansi',
            'id' => 'nama_kuitansi',
            'value' => $this->form_validation->set_value('nama_kuitansi', $stdClass->nama_kuitansi),
            'class' => 'form-control col-md-12'
        );

        $jenispengobatan = $this->mod_app->getPengajuanType(null, null);
        $list_jenispengobatan[null] = "Pilih Tipe Pengobatan";
        foreach ($jenispengobatan as $rowjns) {
            $list_jenispengobatan[$rowjns->jns_aju] = $rowjns->desc_aju;
        }

        $this->data['pengobatan_id'] = array(
            'name' => 'pengobatan_id',
            'id' => 'pengobatan_id',
            'options' => $list_jenispengobatan,
            'value' => $this->form_validation->set_value('pengobatan_id', $stdClass->jns_aju),
            'class' => 'form-control',
        );
        ...
        $this->data['diagnosa'] = array(
            ...
            'maxLength' => '500',
            'rows' => '2',
            'value' => $this->form_validation->set_value('diagnosa', $stdClass->diagnosa)
        );
        ...
        if($id!="0"){
            $linkpath = base_url().'uploads/pengobatan/'.$stdClass->comp_code.'/';
            foreach ($DataAttach as $row_attach) {
                array_push($build_array,
                    array(
                        ...
                        "src" => $linkpath.$row_attach->URL_ATC_OBAT,
                        "is_new" => 0
                    )
                );
            }
        ...
        if(!$show_approve_btn){
            $this->data['nilai_diganti'] = array(
                ...
                'data-politespace-decimal-mark' => '.',
                'disabled' => true,
                'value' => $this->form_validation->set_value('nilai_diganti', $stdClass->nilai_diganti)
            );
        }else{
            $this->data['nilai_diganti'] = array(
                'name' => 'nilai_diganti',
        ...
        return $this->_render_page('presensi/vpengobatan_form', $this->data, false, 'tmpl/vwbacktmpl');
    }
    ...
    public function activate($id)
    {
        ...
        $data['active'] = 1;
        $activation = $this->mobat->update($id, $data);
        if ($activation) {
    ...
    public function deactivate($id = NULL)
    {
        ...
        $data['active'] = 0;
        $deactivate = $this->mobat->update($id, $data);
        if ($deactivate) {
    ...
    public function get_node_nmkuitansi() {
        $out = [];
        ...
        if (!empty($COMPID)) {
            ...
            $filters = [
                ['field' => 'a.compid', 'type' => '=', 'value' => $COMPID],
                ['field' => 'a.nik', 'type' => '=', 'value' => $NIK]
            ];
            $results_self = $this->memployee->get(null, 0, 999999, null, $filters);
            
            // build tree
            $output = [];
            $arr = [];
            
            if ($results_self != null) {
                foreach ($results_self as $row_self) {
                    $arr = [];
                    $arr['seq'] = 0;
                    $arr['nama_kel'] = strtoupper($row_self->emp_name)." - A.N SENDIRI";
                    $output[] = $arr;
                }
            }

            $results = $this->mkel->get(null, 0, 999999, null, $filters);
            if ($results != null) {
                foreach ($results as $row) {
                    $arr['seq'] = $row->seq;
                    $arr['nama_kel'] = strtoupper($row->nama_kel)." - ".strtoupper($row->relasi_kel);
                    $output[] = $arr;
                }
            }
            $out = $output;
