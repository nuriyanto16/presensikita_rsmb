 ...
 * @property Mpelatihan $mlthn
 */

class Pelatihan extends Mst_controller
{
    ...
    function __construct()
    {
        ...
        $this->MOD_ALIAS = "MOD_PRESENSI_PELATIHAN";
        ...
        $this->load->model("Mpelatihan", "mlthn");
    ...
    {
        $this->data['titlehead'] = "Pelatihan";

        $loadfoot['javascript'] = array(
            HTTP_MOD_JS . 'modules/presensi/pelatihan.js'
        );
        ...
        $this->_render_page('vpelatihan_list', $this->data, false, 'tmpl/vwbacktmpl');
    }
    ...
    public function lists()
    {
        ...
        $results = $this->mlthn->get(null, $start, $limit, $order, $filters);
        $totalfiltered = $this->mlthn->get_cnt($filters);
        $totaldata = $this->mlthn->get_cnt();
        ...
        foreach ($results as $row) {
            ...
            if ($this->_edit) {
                $atr_edit['title'] = 'Edit';
                $atr_edit['url'] = 'presensi/pelatihan/edit_form/';
                $atr_edit['class'] = '';
            }
            if ($this->_delete) {
                $atr_del['title'] = 'Hapus';
                $atr_del['url'] = 'presensi/pelatihan/delete/';
                $atr_del['class'] = '';
...
//                anchor("reference/position/activate/{$id}    ", "<i class='fa fa-times text-red'>&nbsp;</i>", array("onclick" => "return confirm('Aktifkan data ?');"));
            ...
            $obj['nm_training'] = $row->nm_training;
            $obj['tgl_start_tr'] = $row->tgl_start_tr;
            $obj['tgl_end_tr'] = $row->tgl_end_tr;
            $obj['jml'] = $row->jml;
            $obj['tempat_tr'] = $row->tempat_tr;
            $obj['nm_lembaga'] = $row->nm_lembaga;
    ...
    public function edit_form()
    {
        ...
        if ($this->data['id'] != "") {
            ...
            $this->data['titlehead'] = "Edit Izin";
        } else {
            $this->data['titlehead'] = "Input Izin";
        }
        ...
        $stdClass->nm_training = null;
        $stdClass->nm_lembaga = null;
        $stdClass->tempat_tr = null;
        $stdClass->periode = date("Y");
        $stdClass->tgl_start_tr = null;
        $stdClass->tgl_end_tr = null;        
        $stdClass->jml = 0;
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $stdClass->nm_training = $this->input->post('nm_training');
            $stdClass->nm_lembaga = $this->input->post('nm_lembaga');
            $stdClass->tempat_tr = $this->input->post('tempat_tr');
            $stdClass->periode = date("Y");
            $stdClass->tgl_start_tr = $this->input->post('tgl_start_tr');
            $stdClass->tgl_end_tr = $this->input->post('tgl_end_tr');
            ...
            if ($this->form_validation->run($this) === TRUE) {
                ...
                $tgl_start_tr = $stdClass->tgl_start_tr;
                $tgl_end_tr = $stdClass->tgl_end_tr;
                $jml = $stdClass->jml;
                $nm_training = $stdClass->nm_training;
                $nm_lembaga = $stdClass->nm_lembaga;
                $tempat_tr = $stdClass->tempat_tr;
                $cnt_file = 0;
                $cnt_file_old = 0;
                $cnt_exist = 0;
                $periode = date("Y");
                ...
                if (in_array($action, array('save'))) {
                    ...
                    if(!empty($fileNames)){ 
                        ...
                        foreach($_FILES['images']['name'] as $key=>$val){ 
                            ...
                            $targetDir =  upload_path.'pelatihan/'.$comp_code.'/';
               ...
               //check to see if we are updating data
                ...
                if ($this->input->post('id')) { // update
                    $isUpdate = true;
                    if ($action === 'save') {
                        $isError = ! $this->mod->InsUpdPelatihan($pengajuan_id, $nik, $comp_code, $periode, 
                ...
                if ($isError) {
                    ...
                    redirect("presensi/pelatihan", 'refresh');
                }
            } else {
        ...
        $loadfoot['javascript'] = array(
            HTTP_MOD_JS . 'modules/presensi/pelatihan_form.js',
        ...
        $this->data['tgl_start_tr'] = array(
            'name' => 'tgl_start_tr',
            'id' => 'tgl_start_tr',
            'type' => 'text',
            'class' => 'form-control',
            'value' => $this->form_validation->set_value('tgl_start_tr', $stdClass->tgl_start_tr)
        );

        $this->data['tgl_end_tr'] = array(
            'name' => 'tgl_end_tr',
            'id' => 'tgl_end_tr',
            'type' => 'text',
            'class' => 'form-control',
            'value' => $this->form_validation->set_value('tgl_end_tr', $stdClass->tgl_end_tr)
        );
        ...
        $this->data['nm_lembaga'] = array(
            ...
            'maxLength' => '500',
            'rows' => '2',
            'value' => $this->form_validation->set_value('nm_lembaga', $stdClass->nm_lembaga)
        );

        $this->data['nm_training'] = array(
            'name' => 'nm_training',
            'id' => 'nm_training',
            'type' => 'text',
            'class' => 'form-control',
            'maxLength' => '500',
            'rows' => '2',
            'value' => $this->form_validation->set_value('nm_training', $stdClass->nm_training)
        );

        $this->data['tempat_tr'] = array(
            'name' => 'tempat_tr',
            'id' => 'tempat_tr',
            'type' => 'text',
            'class' => 'form-control',
            'maxLength' => '500',
            'rows' => '2',
            'value' => $this->form_validation->set_value('tempat_tr', $stdClass->tempat_tr)
        );
        ...
        if($id!="0"){
            $linkpath = base_url().'uploads/pelatihan/'.$stdClass->comp_code.'/';
            foreach ($DataAttach as $row_attach) {
                array_push($build_array,
                    array(
                        ...
                        "src" => $linkpath.$row_attach->URL_ATC_TRAINING,
                        "is_new" => 0
                    )
                );
            }
        ...
        return $this->_render_page('presensi/vpelatihan_form', $this->data, false, 'tmpl/vwbacktmpl');
    }
    ...
    public function activate($id)
    {
        ...
        $data['active'] = 1;
        $activation = $this->mlthn->update($id, $data);
        if ($activation) {
    ...
    public function deactivate($id = NULL)
    {
        ...
        $data['active'] = 0;
        $deactivate = $this->mlthn->update($id, $data);
        if ($deactivate) {
    ...
    public function getExportExcel($fromDate=null, $toDate=null) {
        ...
        // DETAIL
        $results = $this->mlthn->getExport($start_date, $end_date);
        ...
        $html = $this->load->view("vpelatihan_export", $this->data, true);
