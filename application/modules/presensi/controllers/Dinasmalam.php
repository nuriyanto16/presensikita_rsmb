...
class Dinasmalam extends Mst_controller
{
    ...
    function __construct()
    {
        ...
        $this->MOD_ALIAS = "MOD_PRESENSI_DINASMALAM";
        ...
        $this->load->model("Mdinasmalam", "mizn");
        $this->load->model("Izin_model", "mod");
        $this->load->model("Approval_model", "mod_app");
    }

    public function index()
    {
        $this->data['titlehead'] = "Dinas Malam";

        $loadfoot['javascript'] = array(
            HTTP_MOD_JS . 'modules/presensi/dinasmalam.js'
        );
        ...
        $this->_render_page('vdinasmalam_list', $this->data, false, 'tmpl/vwbacktmpl');
    ...
    public function lists()
    {
        ...
        foreach ($results as $row) {
            ...
            if ($this->_edit) {
                $atr_edit['title'] = 'Edit';
                $atr_edit['url'] = 'presensi/dinasmalam/edit_form/';
                $atr_edit['class'] = '';
            }
            if ($this->_delete) {
                $atr_del['title'] = 'Hapus';
                $atr_del['url'] = 'presensi/dinasmalam/delete/';
                $atr_del['class'] = '';
...
//                anchor("reference/position/activate/{$id}    ", "<i class='fa fa-times text-red'>&nbsp;</i>", array("onclick" => "return confirm('Aktifkan data ?');"));
    ...
    public function edit_form()
    {
            ...
            if ($this->form_validation->run($this) === TRUE) {
               ...
               //check to see if we are updating data
                ...
                if ($this->input->post('id')) { // update
                    $isUpdate = true;
                    if ($action === 'save') {
                        $isError = ! $this->mizn->InsUpdizin($pengajuan_id, $nik, $comp_code, $id_abs_type, $remark, $periode, 
                    ...
                    $execute = $this->mizn->InsUpdizin($pengajuan_id, $nik, $comp_code, $id_abs_type, $remark, $periode, 
                        $date, $this->_fyyyymmdd($start_date), $this->_fyyyymmdd($end_date), 
                        $jml, $cnt_file, $params_image_input);
                ...
                if ($isError) {
                    ...
                    redirect("presensi/dinasmalam", 'refresh');
                }
            } else {
        ...
        $loadfoot['javascript'] = array(
            HTTP_MOD_JS . 'modules/presensi/dinasmalam_form.js',
        ...
        $jenisizin = $this->mizn->getIzinType(null, null);
        $list_jenisizin[null] = "Pilih Tipe Pengajuan";
        ...
        if($id!="0"){
            $linkpath = base_url().'uploads/izin/'.$stdClass->comp_code.'/';
        ...
        return $this->_render_page('presensi/vdinasmalam_form', $this->data, false, 'tmpl/vwbacktmpl');
    }
    ...
    public function activate($id)
    {
        ...
        $data['active'] = 1;
        $activation = $this->mizn->update($id, $data);
        if ($activation) {
            $this->mcommon->setLog($this->get_userid(), $this->MOD_ALIAS, $id, "Dinas Malam Diaktifkan");
    ...
    public function deactivate($id = NULL)
    {
        ...
        $data['active'] = 0;
        $deactivate = $this->mizn->update($id, $data);
        if ($deactivate) {
            $this->mcommon->setLog($this->get_userid(), $this->MOD_ALIAS, $id, "Dinas Malam Dinonaktifkan");
    ...
    public function getExportExcel($fromDate=null, $toDate=null) {
        ...
        // DETAIL
        $results = $this->mizn->getExport($start_date, $end_date);
        ...
        header("Content-Disposition: attachment;filename=EXPORT__DINASMALAM_".$start_date."_".$end_date.".xls");
        ...
        $html = $this->load->view("vdinasmalam_export", $this->data, true);
