...
class Ion_auth_model extends CI_Model
{
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $query = $this->db->select('a.id, a.username, a.email, a.password, a.nik, a.active, a.last_login, a.full_name, a.phone,
                                    a.created_date, b.role_id, c.role_name, c.role_alias, emp.compId, a.empid,
                                    d.siteId, emp.unitId, emp.positionId, emp.position_code, a.photo,
                                    a.dt_superadmin, a.dt_admin, a.dt_user')
            ->from($this->tables['users'] . ' a')
            ->join($this->tables['users_groups'] . ' b', 'a.id=b.userid', 'inner')
            ->join($this->tables['groups'] . ' c', 'b.role_id=c.role_id', 'inner')
            ->join(config_item('table_employee') . ' emp', 'a.email=emp.email', 'left')
            ->join(config_item('table_unit') . ' d', 'd.unitId=emp.unitId and d.compid=emp.compid', 'left')
//            ->join(config_item('table_site') . ' e', 'e.siteId=d.siteId', 'left')
...
//        $query = $this->db->get_where($this->tables['groups'], array('role_alias' => $this->config->item('default_group', 'ion_auth')), 1)->row();
//        if (!isset($query->role_id) && empty($groups)) {
...
//        if (isset($default_group->role_id) && empty($groups)) {
//            $groups[] = $default_group->role_id;
//        }
    ...
    public function user($id = NULL)
    {
        ...
        $id || $id = $this->session->userdata($this->config->item('sess_prefix', 'ion_auth') . 'userid');
    ...
    public function get_users_groups($id = FALSE)
    {
        ...
        return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.role_alias, ' . $this->tables['groups'] . '.role_name')
            ->where($this->tables['users_groups'] . '.' . $this->join['users'], $id)
            ->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . '=' . $this->tables['groups'] . '.role_id')
    ...
    public function add_to_group($group_ids, $user_id = false)
    {
        ...
        //if no id was passed use the current users id
        $user_id || $user_id = $this->session->userdata($this->config->item('sess_prefix', 'ion_auth') . 'userid');
    ...
    public function group($id = NULL)
    {
        ...
        if (isset($id)) {
            $this->where($this->tables['groups'] . '.role_id', $id);
        }

        $this->limit(1);
        $this->order_by('role_id', 'desc');
    ...
    public function update($id, array $data)
    {
        ...
        // Filter the data passed (depreceted)
...
//    public function insert_profile_upload($datafile)
//    {
//        $return = false;
//        $this->db->trans_begin();
//        //$this->db->insert($this->tables['users_profile'], $dataUp);
//
//        //-- insert detail file
//        if (count($datafile) > 0) {
...
//                $datafileIn = array(
//                    "userid" => $file['userid'],
//                    "inst_id" => $file['inst_id'],
//                    "address" => $file['address'],
//                    "phone" => $file['phone'],
//                    'photo' => $file['photo']
//                );
//                $this->db->insert($this->tables['users_profile'], $datafileIn);
//            }
//        }
//
//        if ($this->db->trans_status() === FALSE) {
...
//        $this->db->update($this->tables['users_profile'], $data);
//
//        if ($this->db->trans_status() === FALSE) {
//            $this->db->trans_rollback();
//            $return = false;
...
//    public function update_profile_upload($datafile, $id)
//    {
//        $return = false;
//        $this->db->trans_begin();
//        $this->db->where('userid', $id);
//        if (count($datafile) < 1) {
//            $this->db->update($this->tables['users_profile'], $datafile);
//        }
//        //-- insert detail file
//        if (count($datafile) > 0) {
//            foreach ($datafile as $file) {
...
//    }
    ...
    public function update_role($id, $data)
    {
        ...
        $this->db->trans_begin();
        $this->db->where('userid', $id);
        $this->db->update($this->tables['users_groups'], $data);
    ...
    public function delete_user($id)
    {
        ...
        // remove user profile
//        $this->db->delete($this->tables['users_profile'], array('userid' => $id));
    ...
    public function set_session($user)
    {
        ...
        if($user->photo === null) $avatar= "avatar.png";
        ...
        $session_data = array(
            ...
            $this->config->item('sess_prefix', 'ion_auth') . 'nik' => $user->nik,
            $this->config->item('sess_prefix', 'ion_auth') . 'username' => $user->username,
            $this->config->item('sess_prefix', 'ion_auth') . 'email' => $user->email,
            $this->config->item('sess_prefix', 'ion_auth') . 'userid' => $user->id, //everyone likes to overwrite id so we'll use user_id
            'old_last_login' => $user->last_login,
            $this->config->item('sess_prefix', 'ion_auth') . 'full_name' => $user->full_name,
            $this->config->item('sess_prefix', 'ion_auth') . 'roleid' => $user->role_id,
            $this->config->item('sess_prefix', 'ion_auth') . 'rolename' => $user->role_name,
            $this->config->item('sess_prefix', 'ion_auth') . 'rolealias' => $user->role_alias,
            $this->config->item('sess_prefix', 'ion_auth') . 'compId' => $user->compId,
            $this->config->item('sess_prefix', 'ion_auth') . 'siteId' => $user->siteId,
            $this->config->item('sess_prefix', 'ion_auth') . 'unitId' => $user->unitId,
            $this->config->item('sess_prefix', 'ion_auth') . 'empId' => $user->empid,
            $this->config->item('sess_prefix', 'ion_auth') . 'positionId' => $user->positionId,
            $this->config->item('sess_prefix', 'ion_auth') . 'positioncode' => $user->position_code,
            $this->config->item('sess_prefix', 'ion_auth') . 'avatar' => $avatar,
            $this->config->item('sess_prefix', 'ion_auth') . 'ucr_date' => $user->created_date,          
            $this->config->item('sess_prefix', 'ion_auth') . 'isclogin' => TRUE
        );
    ...
    public function login_remembered_user()
    {
         ...
         * ->get($this->tables['users']);
         **/
        $query = $this->db->select('a.' . $this->identity_column . ', a.username, a.nik, a.email, a.id, a.full_name, a.last_login, a.empid,
                                            b.role_id, c.role_name, c.role_alias, e.compId, d.siteId, a.unitId, a.positionId, a.photo')
            ->from($this->tables['users'] . ' a')
            ->join($this->tables['users_groups'] . ' b', 'a.id=b.userid', 'inner')
            ->join($this->tables['groups'] . ' c', 'b.role_id=c.role_id', 'inner')
            ->join(config_item('table_unit') . ' d', 'd.unitId=a.unitId', 'inner')
            ->join(config_item('table_site') . ' e', 'e.siteId=d.siteId', 'inner')
            ->where('a.' . $this->identity_column, get_cookie($this->config->item('identity_cookie_name', 'ion_auth')))
            ->where('a.remember_code', get_cookie($this->config->item('remember_cookie_name', 'ion_auth')))
            ->limit(1)
            ->order_by('a.id', 'desc')
            ->get();
            exit();
    ...
    public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
    {
        ...
        $role_alias = strtolower(str_replace(" ", "", $group_name));
        $existing_group = $this->db->get_where($this->tables['groups'], array('role_alias' => $role_alias))->num_rows();
        if ($existing_group !== 0) {
            $this->set_error('group_already_exists');
            return FALSE;
        }

        $data = array('role_name' => $group_name, 'role_alias' => $role_alias, 'aktif' => 1);
    ...
    public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
    {
        ...
        if (!empty($group_name)) {
            ...
            // bail if the group name already exists
            $existing_group = $this->db->get_where($this->tables['groups'], array('role_id' => $group_id))->row();
            if (isset($existing_group->id) && $existing_group->role_id != $group_id) {
                $this->set_error('group_already_exists');
                return FALSE;
            }
        ...
        $group = $this->db->get_where($this->tables['groups'], array('role_id' => $group_id))->row();
        if ($this->config->item('admin_group', 'ion_auth') === $group->role_alias && $group_name !== $group->role_name) {
        ...
        $this->db->update($this->tables['groups'], $data, array('role_id' => $group_id));
    ...
    public function delete_group($group_id = FALSE)
    {
        ...
        $this->db->delete($this->tables['groups'], array('role_id' => $group_id));
