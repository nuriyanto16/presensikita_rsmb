...
class Absensi_model extends CI_Model
{
    ...
    public function getAbsensiSummary($id = null, $comp_code = null, $period_awal = null, $period_akhir = null) // $id defaultnya adalah null
    {
        ...
        $this->db->select("A.ID_ABS_TYPE, B.ABS_TYPE_DESC , 
        SUM(IF(A.ID_ABS_TYPE IN(1,2) AND A.JML_JAM_KERJA > 0, 1,0)) AS JML_HADIR,
        SUM(IF(A.ID_ABS_TYPE IN(1,2) AND A.JAM_MASUK = '0000-00-00 00:00:00' AND A.JAM_MASUK = '0000-00-00 00:00:00', 1,0)) AS JML_MANGKIR,
        SUM(IF(A.ID_ABS_TYPE IN(1,2) AND A.JML_TERLAMBAT > 0, 1,0)) AS JML_TERLAMBAT,
        SUM(IF(A.ID_ABS_TYPE NOT IN(1,2), 1,0)) AS JML");
        ...
        //if (!empty($start_date)  &&  !empty($end_date)  )  {
            $this->db->where("A.TANGGAL >=", $period_awal );
            $this->db->where("A.TANGGAL <=", $period_akhir );  
        //}
