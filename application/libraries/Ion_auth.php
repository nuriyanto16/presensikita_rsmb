...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		{
                    $this->email->initialize($email_config); //remark by kiq
                    //$this->load->library('bss_email');
		}
				...
				else
				{
					...
					else
					{
						...
						return FALSE;
                                        ...
                                        /*
                                        $this->bss_email->initialize();
					$this->bss_email->AddTo($user->email);
					$this->bss_email->Subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_forgotten_password_subject'));
					$this->bss_email->Message($message);

					$resSend = $this->bss_email->SendEmail();
                                        print_r($resSend ); exit;
                                        if ($resSend['success'] == true)
					{
						$this->set_message('forgot_password_successful');
						return TRUE;
					}
		...
		$identity = $this->config->item('identity', 'ion_auth');
                $this->session->unset_userdata(
                        array(
                            $identity => '',
                            'id' => '',
							$this->config->item('sess_prefix', 'ion_auth') .'username' => '',
							$this->config->item('sess_prefix', 'ion_auth') .'NIK' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'email' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'userid' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'full_name' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'phone' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'roleid' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'rolename' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'rolealias' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'compId' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'branchId' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'unitId' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'positionId' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'avatar' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'ucr_date' => '',
                            $this->config->item('sess_prefix', 'ion_auth') .'isclogin' => FALSE
                        )
                );
	...
	{
		$user_id = $this->session->userdata($this->config->item('sess_prefix', 'ion_auth') .'userid');
		if (!empty($user_id))
		{
			return $user_id;
		}
	...
	public function is_superadmin($id=false)
	{
		...
		$sa_group = $this->config->item('superadmin_group', 'ion_auth');
	...
	public function in_group($check_group, $id=false, $check_all = false)
	{
		...
		else
		{
			...
			{
				$groups_array[$group->id] = $group->role_alias;
			}
